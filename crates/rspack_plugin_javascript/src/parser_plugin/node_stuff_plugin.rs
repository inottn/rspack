use rspack_core::{
  CachedConstDependency, ConstDependency, JavascriptParserImportMeta, NodeDirnameOption,
  NodeFilenameOption, NodeGlobalOption, RuntimeGlobals, get_context, parse_resource,
};
    ...
    match for_name {
      ...
      expr_name::IMPORT_META_FILENAME => {
        ...
        if matches!(
          ...
          Some(JavascriptParserImportMeta::Disable)
        ) {
          return None;
        }
        // Skip if node: false or node.filename is disabled
